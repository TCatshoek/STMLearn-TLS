...
class TLSTraceBuilder:
    ...
    # Try to handle unimplemented methods by just writing them to simple XML tags
    def __getattr__(self, name):
        def handle_unknown(*args):
            if not args:
                self.add_send(ET.Element(name))
            else:
                assert False, f"Unknown method {name}"
        return handle_unknown

    # How to roll print and file outputs into one?
    def print(self):
...
# This architecture is kinda terrible, but let's get something up and running first,
...
class TLSAttackerMapper:
    def __init__(self, tlsa_path, addr='localhost', port='4433', enable_heartbeat=False):
        ...
        self.mapping = {
            ...
            # "ClientHelloHeartBeat": lambda: self.tb.ClientHello(extensions=[
            #     'ECPointFormat',
            #     'HeartbeatExtension',
            #     'EllipticCurves'
            # ]),
            ...
            #"ApplicationHenlo":     lambda: self.tb.Application("henlo"),
            # "Certificate":          self.tb.Certificate
        }
...
#
# ClientHello(RSA and DHE)
# Certificate(RSA and empty)
# ClientKeyExchange
# ClientCertificateVerify
# ChangeCipherSpec
# Finished
# ApplicationData(regular and empty)
# HeartbeatRequest
# HeartbeatResponse

if __name__ == "__main__":
    tlsattackerpath = '/home/tom/projects/tlsattacker/TLS-Attacker/apps/TLS-Client.jar'
    tam = TLSAttackerMapper(tlsattackerpath)
    ...
    #
    xmlb = TLSTraceBuilder()
    xmlb.ClientHello(extensions=[
        'ECPointFormat',
        'HeartbeatExtension',
        'EllipticCurves'
    ])
    xmlb.GenericReceive()
    xmlb.RSAClientKeyExchange(computations=[])
    xmlb.GenericReceive()
    # xmlb.print()
    #
